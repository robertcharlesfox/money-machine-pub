...
class ScrapeGallupCongressHomePage extends Job implements SelfHandling, ShouldQueue
{
    ...
    public function handle()
    {
        ...
        if (strlen($this->pollster->new_poll_update_text) < 3 && ! $this->pollster->is_likely_final_for_week) {
            ...
            echo $time . ' Beginning Scrape for Gallup Congress via HomePage' . "\n";
            // Make a new bot and call the scrape
            $bot = new ScraperBot();
            $bot->scrapeGallupHomePage($this->pollster, 'Congress');
        }
        else {
