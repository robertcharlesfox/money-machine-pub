...
var LessonEdit = {
    ...
    initialize: function(bindEventHandlers, manageMode) {
        ...
        LessonEdit.manageMode = manageMode;
        ...
        $('#select-date').datepicker({
            ...
            onSelect: function(dateText, instance) {
                LessonEdit.getAvailableHours(dateText);
                LessonEdit.updateConfirmFrame();
            }
        });
        ...
        if (bindEventHandlers) {
            LessonEdit.bindEventHandlers();
        }

        // If the manage mode is true, the appointments data should be
        // loaded by default.
        if (LessonEdit.manageMode) {
            LessonEdit.applyAppointmentData(GlobalVariables.appointmentData,
    ...
    bindEventHandlers: function() {
        ...
        $('#select-provider').change(function() {
            LessonEdit.getAvailableHours(Date.today().toString('dd-MM-yyyy'));
            LessonEdit.updateConfirmFrame();
        });
        ...
        $('#select-service').change(function() {
            ...
            LessonEdit.getAvailableHours($('#select-date').val());
            LessonEdit.updateConfirmFrame();
            LessonEdit.updateServiceDescription($('#select-service').val(), $('#service-description'));
        });
        ...
        $('.button-next').click(function() {
            ...
            if ($(this).attr('data-step_index') === '3') {
                if (!LessonEdit.validateCustomerForm()) {
        ...
        /**
         * Event: Schedule Lesson Button "Clicked"
         *
         * Show the Schedule Lesson wizard.
         */
        $('#button-schedule-lesson').click(function() {
            $('#section-appointment-wizard').show('fade');
        });
        ...
        if (LessonEdit.manageMode) {
            ...
            $('#cancel-appointment').click(function(event) {
                ...
                GeneralFunctions.displayMessageBox('cancel_appointment_title',
                        'write_appointment_removal_reason', dialogButtons);
        ...
        $('#book-appointment-submit').click(function(event) {
            LessonEdit.registerAppointment();
        });
    ...
    getAvailableHours: function(selDate) {
        ...
        var appointmentId = (LessonEdit.manageMode)
                ? GlobalVariables.appointmentData['id'] : undefined;

        // Make ajax post request and get the available hours.
        var postUrl = GlobalVariables.baseUrl + '/ajax_get_available_hours';
        ...
        var postData = {
            ...
            'manage_mode': LessonEdit.manageMode,
    ...
    updateConfirmFrame: function() {
        ...
        postData['appointment'] = {
            'start_datetime': $('#select-date').datepicker('getDate').toString('yyyy-MM-dd')
                                    + ' ' + selectedHour + ':00',
            'end_datetime': LessonEdit.calcEndDatetime(selectedHour),
            'notes': $('#notes').val(),
        ...
        postData['manage_mode'] = LessonEdit.manageMode;

        if (LessonEdit.manageMode) {
            postData['appointment']['id'] = GlobalVariables.appointmentData['id'];
            postData['customer']['id'] = GlobalVariables.customerData['id'];
        } else {
            // todo - new variable here, for the current user
            postData['appointment']['id_users_customer'] = GlobalVariables.customerData['id'];
        }
    ...
    registerAppointment: function() {
        ...
        var postUrl = GlobalVariables.baseUrl + '/ajax_register_appointment',
            $layer = $('<div/>');
        ...
        $.ajax({
            ...
            .done(function(response) {
                ...
                window.location.replace(GlobalVariables.baseUrl
                    + '/book_success/' + response.appointment_id);
            })
